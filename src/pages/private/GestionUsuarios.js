...
export function GestionUsuarios() {
  ...
  const filtrar = (terminoBusqueda) => {
    let resultadosBusqueda = registros.filter((elemento) => {
      let elemento1 = "";
      if (
        elemento.numDoc
          .toString()
  ...
  const displayUsers =
    tablaMod &&
    tablaMod.slice(pagesVisited, pagesVisited + usersPerPage).map((item) => (
      <tr key={item._id}>
        <td>{item.numDoc}</td>
        <td>{item.nombre}</td>
  ...
  return (
    <div>
      <div>
        ...
        <Link to="/crear" className="btn btn-success mr-2 btn-sm">
          <small>
        ...
        <form className=" d-sm-inline-block form-inline   my-md-2 navbar-search float-right ">
          ...
          <div className="input-group">
            <input
              type="text"
              className="form-control bg-succes border-1 small"
